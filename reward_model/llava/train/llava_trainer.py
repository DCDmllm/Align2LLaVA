...
def get_peft_state_maybe_zero_3(named_params, bias):
    if bias == "none":
        to_return = {k: t for k, t in named_params if "lora_" in k or "modules_to_save" in k}
    elif bias == "all":
        to_return = {k: t for k, t in named_params if "lora_" in k or "bias" in k or "modules_to_save" in k}
    elif bias == "lora_only":
        to_return = {}
        maybe_lora_bias = {}
        lora_bias_names = set()
...
class LLaVATrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        scores1, scores2 = scores[:batch_size], scores[batch_size:]
        loss = -nn.functional.logsigmoid(scores1 - scores2).mean()
